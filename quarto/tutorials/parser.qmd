...
The result is a Vector of analyses. Each `Analysis` (from the `CitableParserBuilder` package) includes a *morphological form* object and an identifier for a *lexeme* (or vocabulary item). You can use the `latinForm` and `lexemeurn` functions to extract these from an `Analysis`; for a human-readable string value, use the `label` function on the result. E.g., label the first form in the result:
