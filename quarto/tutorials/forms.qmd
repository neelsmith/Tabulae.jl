...
Start by repeating the steps from the [introductory tutorial](./parser.qmd) to compile a parser:
...
We can take advantage of this in normal Julia operations on collections of analyses. For instance, we could separate out all the *plural* analyses for *agricolae*:
...
Note that this *not* a traditional "part of speech." Rather, the subtypes of `LatinMorphologicalForm` are *analytical types*. Verbs have distinct analytical types for finite, infinitive and participle forms, for example.
