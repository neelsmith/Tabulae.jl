...
Forms and stems are citable objects. They implement both the citable trait and the CEX serializable trait of the `CitableBase` package.

**ADD LINK TO QUARTO DOCS FOR BASE**
...
- `formurn`.  The `formurn` function is implemented by the stem type for regular inflected forms; for irregular forms and uninflected forms (includling pronouns), the `formurn` function is implemented for the rule type.
...
Because this is a pronoun type, we apply `formurn` to the stem.
...
- `formurn`: from CPB package, gets a URN value for the ofrm of an Analysis
